...
public class Blomgren {
    ...
    public void run(int nombreIterations) {
        ...
        double ch[][] = new double[width + 4][height + 4];
        double emss[][] = new double[width + 4][height + 4];
...
// COUCHE ROUGE -------------------------------------------------------------------------------------------------
            for (x = 1; x < width - 1; x++) {
                for (y = 1; y < height - 1; y++) {
                    ...
                    SapirR[x][y] = Math.sqrt(landaPLUS + landaMOINS);
                    Sapir_Rouge[x][y] = ((Ixx[x][y] * Iy[x][y] * Iy[x][y] - 2 * Ixy[x][y] * Ix[x][y] * Iy[x][y] + Iyy[x][y] * Ix[x][y] * Ix[x][y]) / (Ix[x][y] * Ix[x][y] + Iy[x][y] * Iy[x][y]));
                    Sapir_Rouge[x][y] *= fctSeuille(SapirR[x][y]);
                }
            }
            // Traitement  R 			
            for (x = 0; x < width + 4; x++) {
...
// COUCHE VERT --------------------------------------------------------------------------------------------------------
            for (x = 1; x < width - 1; x++) {
                for (y = 1; y < height - 1; y++) {
                    ...
                    double landaPLUS = (G11[x][y] + G22[x][y] + Math.sqrt(((G11[x][y] - G22[x][y]) * (G11[x][y] - G22[x][y])) + 4 * G12[x][y] * G12[x][y])) / 2;
                    double landaMOINS = (G11[x][y] + G22[x][y] - Math.sqrt(((G11[x][y] - G22[x][y]) * (G11[x][y] - G22[x][y])) + 4 * G12[x][y] * G12[x][y])) / 2;
                    SapirV[x][y] = Math.sqrt(landaPLUS + landaMOINS);
                    Sapir_Vert[x][y] = ((Ixx[x][y] * Iy[x][y] * Iy[x][y] - 2 * Ixy[x][y] * Ix[x][y] * Iy[x][y] + Iyy[x][y] * Ix[x][y] * Ix[x][y]) / (Ix[x][y] * Ix[x][y] + Iy[x][y] * Iy[x][y]));
                    Sapir_Vert[x][y] *= fctSeuille(SapirV[x][y]);
                }
            }
            // Traitement  R 			
            for (x = 0; x < width + 4; x++) {
...
// COUCHE BLUE --------------------------------------------------------------------------------------------------------
            for (x = 1; x < width - 1; x++) {
                for (y = 1; y < height - 1; y++) {
                    ...
                    double landaPLUS = (G11[x][y] + G22[x][y] + Math.sqrt(((G11[x][y] - G22[x][y]) * (G11[x][y] - G22[x][y])) + 4 * G12[x][y] * G12[x][y])) / 2;
                    double landaMOINS = (G11[x][y] + G22[x][y] - Math.sqrt(((G11[x][y] - G22[x][y]) * (G11[x][y] - G22[x][y])) + 4 * G12[x][y] * G12[x][y])) / 2;
                    SapirB[x][y] = Math.sqrt(landaPLUS + landaMOINS);
                    Sapir_Bleu[x][y] = ((Ixx[x][y] * Iy[x][y] * Iy[x][y] - 2 * Ixy[x][y] * Ix[x][y] * Iy[x][y] + Iyy[x][y] * Ix[x][y] * Ix[x][y]) / (Ix[x][y] * Ix[x][y] + Iy[x][y] * Iy[x][y]));
                    Sapir_Bleu[x][y] *= fctSeuille(SapirB[x][y]);
                }
            }
            // Traitement  R 			
            for (x = 0; x < width + 4; x++) {
...
//---------------------------------------------------------------------------------------------------------------------
        ...
        System.out.println("traitement est finit ");
        try {
            File file_out = new File("Blomgren.jpg");
